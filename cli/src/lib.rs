...
use solana_transaction_status_client_types::{EncodedConfirmedTransactionWithStatusMeta, EncodedTransaction, UiCompiledInstruction, UiInstruction, UiMessage, UiRawMessage, UiTransactionEncoding};
use anchor_client::Cluster;
use anchor_lang::{idl::{IdlAccount, IdlInstruction, ERASED_AUTHORITY}, prelude::Account};
...
fn extract_first_compiled_instruction(tx: &EncodedConfirmedTransactionWithStatusMeta) -> Option<UiCompiledInstruction> {
    if let EncodedTransaction::Json(ui_tx) = &tx.transaction.transaction {
        // println!("ui_tx: {:?}", ui_tx.message);
        if let UiMessage::Raw(parsed_msg) = &ui_tx.message { // Use `Parsed` instead of `Raw`
            // println!("paresed {:?}", parsed_msg);
            return Some(parsed_msg.instructions.first().unwrap().clone())
        }
    }
    None // Return None if no instructions exist
}

fn extract_ui_raw_message(tx: &EncodedConfirmedTransactionWithStatusMeta) -> Option<UiRawMessage> {
    if let EncodedTransaction::Json(ui_tx) = &tx.transaction.transaction {
        // println!("ui_tx: {:?}", ui_tx.message);
        if let UiMessage::Raw(parsed_msg) = &ui_tx.message { // Use `Parsed` instead of `Raw`
            // println!("paresed {:?}", parsed_msg);
            return Some(parsed_msg.clone())
        }
    }
    None // Return None if no instructions exist
}
...
fn extract_first_compiled_instruction_data_and_removed_discrimination_bytes(tx: &EncodedConfirmedTransactionWithStatusMeta) -> Option<Vec<u8>> {
    if let EncodedTransaction::Json(ui_tx) = &tx.transaction.transaction {
        if let UiMessage::Raw(parsed_msg) = &ui_tx.message {
            // return Some(parsed_msg.instructions.first().unwrap().data.clone())
            let encoded_data = parsed_msg.instructions.first()?.data.clone();
            
            // println!("encoded_data: {:?}", encoded_data);
            let decoded_bytes = bs58::decode(&encoded_data).into_vec().ok()?;
            let pure_data = decoded_bytes[13..].to_vec();
            // println!("decoded data: {:?}", decoded_bytes);
            return Some(pure_data);
            // if decoded_bytes.len() > 13 {
            //     return Some(decoded_bytes[13..].to_vec()); // Strip the first 13 bytes
            // }
        }
    }
    None // Return None if no instructions exist
...
//         transactions.extend(results.into_iter().flatten());
//     }

//     transactions
// }

fn contains_log_message(tx: &EncodedConfirmedTransactionWithStatusMeta, target: String) -> bool {
    return tx.transaction.meta
    .as_ref() // Borrow instead of move
    .map_or(false, |meta| {
        meta.log_messages
            .as_ref() // Borrow log_messages instead of move
            .map_or(false, |logs| {
                logs.iter().any(|log| 
                    // log.contains("Program log: Instruction: IdlSetBuffer")
                    log.contains(&target)
                )
            })
    });
...
fn partition_transactions_by_log(
    transactions: Vec<EncodedConfirmedTransactionWithStatusMeta>, // Take ownership of transactions
) -> (
    Vec<EncodedConfirmedTransactionWithStatusMeta>, // Return owned data
    Vec<EncodedConfirmedTransactionWithStatusMeta>, // Return owned data
) {
    let mut idl_write_txs = Vec::new();
    let mut idl_set_buffer_txs = Vec::new();

    for tx in transactions {
        if contains_log_message(&tx, "IdlWrite".to_string()) {
            idl_write_txs.push(tx); // Clone to store in new Vec
        }
        else if contains_log_message(&tx, "IdlSetBuffer".to_string()) {
            idl_set_buffer_txs.push(tx);
        }
    }

    (idl_write_txs, idl_set_buffer_txs)
}
...
fn fetch_transactions(url: String, sigs: Vec<String>) -> Vec<EncodedConfirmedTransactionWithStatusMeta> {
    let client = Arc::new(RpcClient::new(url.to_string())); // Shared client
    let rt = Runtime::new().unwrap(); // Create a Tokio runtime

    let mut transactions = Vec::new();

    for chunk in sigs.chunks(10) {
        // println!("Fetching batch of {} transactions...", chunk.len());
        ...
        let handles: Vec<_> = chunk.iter()
            .filter_map(|sig| {
                ...
                Some(rt.spawn(async move {
                    ...
                    client_clone.get_transaction(&signature, UiTransactionEncoding::Json).ok()
                }))
            })
            .collect();
        ...
        let results: Vec<_> = handles.into_iter()
            .filter_map(|handle| rt.block_on(handle).ok().flatten())
            .collect();

        // println!("Fetched {} transactions in batch.", results.len());
...
fn get_idl_write_txs_vec_from_set_buffer_tx<'a>(
    client: &RpcClient,
    idl_set_buffer_tx: &'a EncodedConfirmedTransactionWithStatusMeta, // Process only one transaction
    idl_account_addr: &String,
) -> Vec<EncodedConfirmedTransactionWithStatusMeta> { // Return Vec directly
    let ui_raw_message = extract_ui_raw_message(idl_set_buffer_tx).unwrap();
    let mut buffer_account_str = ui_raw_message.account_keys.get(2).unwrap();
    
    if buffer_account_str == idl_account_addr {
        buffer_account_str = ui_raw_message.account_keys.get(1).unwrap();
    }
    
    let buffer_account = Pubkey::from_str(buffer_account_str).unwrap();

    let sigs_response = client.get_signatures_for_address(&buffer_account).unwrap();
    let sigs = sigs_response
        .iter()
        .map(|signature| signature.signature.clone())
        .collect::<Vec<String>>();

    // Fetch new transactions
    let new_transactions = fetch_transactions(
        "https://attentive-special-gas.solana-mainnet.quiknode.pro/bdaab501c5ecec1ab507b6aa579bcb278ee53af8".to_string(),
        sigs,
    );

    // Get references to newly added transactions
    let (idl_write_txs_of_idl_account, _) = partition_transactions_by_log(new_transactions);

    idl_write_txs_of_idl_account // Return the vector directly
}
...
// fn fetch_transactions(client: RpcClient, sigs: Vec<String>) -> Vec<_> {
//     sigs.chunks(5) // Process in batches of 5
//         .flat_map(|chunk| {
//             let handles: Vec<_> = chunk.iter()
//                 .map(|sig| {
...
//                         let signature = Signature::from_str(&sig).ok()?;
//                         client.get_transaction(&signature, UiTransactionEncoding::Json).ok()
//                     })
//                 })
//                 .collect();

//             handles.into_iter()
//                 .filter_map(|handle| handle.blocking_recv().ok().flatten()) // Await the results
//                 .collect::<Vec<_>>() // Collect successful transactions
//         })
//         .collect()
// }
...
fn fetch_idl(cfg_override: &ConfigOverride, idl_addr: Pubkey) -> Result<serde_json::Value> {
    // println!("fetch idl"); 
    let mut url = match Config::discover(cfg_override)? {
    ...
    url = "https://attentive-special-gas.solana-mainnet.quiknode.pro/bdaab501c5ecec1ab507b6aa579bcb278ee53af8".to_string();
    // url = "https://crimson-summer-mansion.solana-devnet.quiknode.pro/2c63100a69ab5fd5de16cea3df5312d9e29addbb".to_string();
    // println!("URL: {:?}", url); 
    let client = create_client(url);

    let mut idl_account_addr = idl_addr;
    let mut account = client.get_account(&idl_addr)?;
    if account.executable {
        idl_account_addr = IdlAccount::address(&idl_addr);
        
        // println!("idl address: {:?}", idl_account_addr);
        account = client.get_account(&idl_account_addr)?;
    }
    let sigs_response = client.get_signatures_for_address(&idl_account_addr).unwrap();
    let sigs = sigs_response.iter().map(|signature| signature.signature.clone()).collect::<Vec<String>>();

    // println!("sigs: {:?}", sigs);
    // println!("sigs len: {:?}", sigs.len());
    // let tx = client.get_transaction(&Signature::from_str(&sigs.get(0).unwrap().signature).unwrap(), UiTransactionEncoding::Json).unwrap();
    

    let batch_size = 5;

    // let transactions = stream::iter(sigs.chunks(batch_size))
    //     .then(|chunk| async {
    //         let futures = chunk.iter().map(|sig| {
    //             async move {
    //                 let signature = Signature::from_str(sig).ok()?;
    //                 client.get_transaction(&signature, UiTransactionEncoding::Json).await
    //             }
    //         });

    //         futures::future::join_all(futures).await
    //     })
    //     .flat_map(stream::iter) // Flatten the batch results
    //     .collect::<Vec<_>>()
    //     .await;

    // let transactions = fetch_transactions(&client, sigs.into_iter().map(|sig_res| sig_res.signature.clone()).collect()).await;


    // let transactions: Vec<_> = sigs.iter()
    //     .filter_map(|sig| {
    ...
    println!("🔍 Searching for IDL history of program: {}", idl_addr);
    println!("📡 Retrieving additional data for the entry 1 ...");    
    let transactions: Vec<_> = fetch_transactions("https://attentive-special-gas.solana-mainnet.quiknode.pro/bdaab501c5ecec1ab507b6aa579bcb278ee53af8".to_string(), sigs);
    // let transactions: Vec<_> = fetch_transactions("https://crimson-summer-mansion.solana-devnet.quiknode.pro/2c63100a69ab5fd5de16cea3df5312d9e29addbb".to_string(), sigs);
    

    let (idl_write_txs_of_idl_account, idl_set_buffer_txs) = partition_transactions_by_log(transactions);
    // let idl_write_txs_of_idl_account = filter_transactions_by_log(&transactions, "IdlWrite");

    
    let mut idl_write_txs_vec: Vec<Vec<EncodedConfirmedTransactionWithStatusMeta>> = Vec::new();

    // let hihi = get_idl_write_txs_vec_from_set_buffer_tx(&client, idl_set_buffer_txs.get(0).unwrap(), &idl_account_addr.to_string());

    let concatenated_idl_write_txs = idl_write_txs_of_idl_account.iter()
        .filter_map(|tx| extract_first_compiled_instruction_data_and_removed_discrimination_bytes(tx)) // Extract Vec<u8> if available
        .flatten() // Flatten into a single Vec<u8>
        .collect::<Vec<u8>>();
    let compressed_bytes = &concatenated_idl_write_txs[0..0 + concatenated_idl_write_txs.len()];
    let mut z = ZlibDecoder::new(compressed_bytes);
    let mut s = Vec::new();
    z.read_to_end(&mut s)?;
    let idl: Result<serde_json::Value> = serde_json::from_slice(&s[..]).map_err(Into::into);
    // println!("idl {:?}:", idl2)
    ...
    match idl {
        Ok(json_data) => {
            match serde_json::to_string_pretty(&json_data) {
                Ok(pretty_json) => {
                    let program_id = idl_addr.to_string();
                    let dir_path = format!("/home/conheo1428/anchor/cli/data/{}", program_id);
                    let file_path = format!("{}/{}.json", dir_path, idl_write_txs_of_idl_account.get(0).unwrap().slot);
                    ...
                    println!(
                        "One IDL publish from slot {} has been saved at: file://wsl.localhost/Ubuntu-24.04/home/conheo1428/anchor/cli/data/{}/{}.json", 
                        idl_write_txs_of_idl_account.get(0).unwrap().slot, program_id, idl_write_txs_of_idl_account.get(0).unwrap().slot
                    );
                }
                Err(e) => eprintln!("Failed to format JSON: {:?}", e),
            }
        }
        Err(e) => eprintln!("Failed to parse IDL JSON: {:?}", e),
    }
    // idl_write_txs_vec.push(idl_write_txs_of_idl_account);
    for (index, idl_set_buffer_tx) in idl_set_buffer_txs.iter().enumerate() {
        // println!("🔍 Searching for IDL history of program: {}", idl_addr);
        println!("📡 Retrieving additional data for IDL entry {} ...", index + 2);   
        let idl_write_txs = get_idl_write_txs_vec_from_set_buffer_tx(&client, idl_set_buffer_tx, &idl_account_addr.to_string());
        // println!("push {:?}", idl_write_txs.len());
        // idl_write_txs_vec.push(idl_write_txs);
        let concatenated_idl_write_txs = idl_write_txs.iter()
            .filter_map(|tx| extract_first_compiled_instruction_data_and_removed_discrimination_bytes(tx)) // Extract Vec<u8> if available
            .flatten() // Flatten into a single Vec<u8>
            .collect::<Vec<u8>>();
        let compressed_bytes = &concatenated_idl_write_txs[0..0 + concatenated_idl_write_txs.len()];
            ...
            Ok(json_data) => {
                match serde_json::to_string_pretty(&json_data) {
                    Ok(pretty_json) => {
                        let program_id = idl_addr.to_string();
                        let dir_path = format!("/home/conheo1428/anchor/cli/data/{}", program_id);
                        let file_path = format!("{}/{}.json", dir_path, idl_set_buffer_tx.slot);
                        ...
                        println!(
                            "One IDL publish from slot {} has been saved at: file://wsl.localhost/Ubuntu-24.04/home/conheo1428/anchor/cli/data/{}/{}.json", 
                            idl_set_buffer_tx.slot, program_id, idl_set_buffer_tx.slot
                        );
    ...
    println!("Done :3");
    // println!("{:?}, {:?}", idl_write_txs_vec.len(), 
    // idl_write_txs_vec.iter().map(|v| v.len()).collect::<Vec<usize>>());

    // println!("hihi {:?}", hihi.len());
    // idl_set_buffer_txs.iter().for_each(|tx| {
    //     let ui_raw_message = extract_ui_raw_message(tx).unwrap();
    //     let mut buffer_account_str = ui_raw_message.account_keys.get(2).unwrap();
    //     if buffer_account_str == &idl_account_addr.to_string() {
    //         buffer_account_str = ui_raw_message.account_keys.get(1).unwrap();
    //     }
    //     let buffer_account = Pubkey::from_str(buffer_account_str).unwrap();
    ...
    //     let transactions: Vec<_> = fetch_transactions(
    //         "https://attentive-special-gas.solana-mainnet.quiknode.pro/bdaab501c5ecec1ab507b6aa579bcb278ee53af8".to_string(),
    //         sigs,
    //     );
    
    //     let (idl_write_txs_of_buffer_account, _) = partition_transactions_by_log(&transactions);
    
    //     if !idl_write_txs_of_buffer_account.is_empty() {
    //         idl_write_txs_vec.push(idl_write_txs_of_buffer_account.to_vec()); // Use to_vec() instead of clone()
    //     }
    // });
    
    
    
    // if !idl_write_txs_of_idl_account.is_empty() {
    //     idl_write_txs_vec.push(idl_write_txs_of_idl_account.clone());
    // }
    
    // let idl_set_buffer_txs = filter_transactions_by_log(&transactions, "IdlSetBuffer");
    

    // println!("idl write txs len: {:?}", idl_write_txs_of_idl_account.len());
    // println!("idl set buffer txs len: {:?}", idl_set_buffer_txs.len());
    // let concatnate_idl_write_txs: String = idl_write_txs.iter()
    // .filter_map(|tx| extract_first_compiled_instruction_data(tx)) // Extract data if available
    // .collect(); // Concatenate all strings
    // let concatenated_idl_write_txs = idl_write_txs_of_idl_account.iter()
    //     .filter_map(|tx| extract_first_compiled_instruction_data_and_removed_discrimination_bytes(tx)) // Extract Vec<u8> if available
    //     .flatten() // Flatten into a single Vec<u8>
    //     .collect::<Vec<u8>>();
    ...
    // println!("Concatenated IDL Write Instructions: {:?}", concatenated_idl_write_txs);
    // let tx = client.get_transaction(&Signature::from_str(&sigs.get(0).unwrap().signature).unwrap(), UiTransactionEncoding::Json).unwrap();
    // let is_set_buffer_tx = tx.transaction.meta.unwrap().log_messages.
    // let is_set_buffer_tx = tx.transaction.meta
    // .as_ref() // Borrow instead of move
    // .map_or(false, |meta| {
    //     meta.log_messages
    //         .as_ref() // Borrow log_messages instead of move
    ...
    //             logs.iter().any(|log| 
    //                 // log.contains("Program log: Instruction: IdlSetBuffer")
    //                 log.contains("IdlSetBuffer")
    //             )
    //         })
    // });
    // let is_set_buffer_tx = contains_log_message(&tx, "IdlSetBuffer");
    
    // println!("is set buffer: {:?}", is_set_buffer_tx);

    // // Cut off account discriminator.
    // println!("tx {:?}", extract_first_compiled_instruction(&tx));
    // println!("tx {:?}", tx);
    // let compressed_bytes = &concatenated_idl_write_txs[0..0 + concatenated_idl_write_txs.len()];
    // let mut z = ZlibDecoder::new(compressed_bytes);
    // let mut s = Vec::new();
    // z.read_to_end(&mut s)?;
    // let idl = serde_json::from_slice(&s[..]).map_err(Into::into);
    // println!("idl {:?}", idl);
    // return idl;
    ...
    // println!("Returning empty JSON: {:?}", empty_json);
...
// fn deserialize_idl<T: DeserializeOwned>(idl_data: Vec<u8>) -> Result<T, Box<dyn std::error::Error>> {
//     let mut decoder = ZlibDecoder::new(&idl_data[..]);
//     let mut decompressed_bytes = Vec::new();
//     decoder.read_to_end(&mut decompressed_bytes)?;
    
//     let idl: T = serde_json::from_slice(&decompressed_bytes)?;
//     Ok(idl)
// }
...
fn idl_write(
    ...
    let idl2: Idl = serde_json::from_slice(&decompressed_bytes)?;
    println!("idl {:?}", idl2);
    ...
    while offset < idl_data.len() {
        ...
        let data = {
            ...
            println!("Step {:?}", &idl_data[start..end]);
            // println!("Step {:?} ", idl_data[start..end].to_vec());
...
fn set_workspace_dir_or_exit() {
    ...
    println!("{:?}", ConfigOverride::default()); 
    match d {
        None => {
